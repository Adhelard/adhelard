// fileName: Contact.jsx (SIAP PAKAI)
...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    if (e) e.preventDefault(); // Mencegah refresh jika dipicu dari event form standar

    // --- 2. Validasi Manual ---
    if (!form.name || !form.email || !form.message) {
      alert('Mohon isi semua kolom (Nama, Email, dan Pesan) terlebih dahulu.');
      return;
    }
    ...
    // Mengambil ID rahasia dari file .env
    const SERVICE_ID = import.meta.env.VITE_EMAILJS_SERVICE_ID;
    const TEMPLATE_ID = import.meta.env.VITE_EMAILJS_TEMPLATE_ID;
    const PUBLIC_KEY = import.meta.env.VITE_EMAILJS_PUBLIC_KEY;

    // --- 3. Eksekusi Pengiriman EmailJS ---
    emailjs
      ...
      <main className="mx-auto w-full max-w-5xl px-6 py-24 sm:py-32">
        ...
        >
          ...
          >
            <MagneticButton 
              type="submit" 
              onClick={handleSubmit} // Akan memicu validasi -> kirim ke EmailJS
              disabled={loading}
